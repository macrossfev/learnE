...
class StudyPlanActivity : AppCompatActivity() {
    ...
    private fun showPlanList() {
        ...
        fun rebuildList() {
            val currentPlans = UserPreferencesRepository.getAllPlanSaves()
            if (currentPlans.isEmpty()) {
                dialogRef[0]?.dismiss()
                AlertDialog.Builder(this@StudyPlanActivity)
                    .setTitle("没有存档")
                    .setMessage("暂无可用的存档，请先创建新的学习。")
                    .setPositiveButton("确定", null)
                    .show()
            ...
            buildPlanCards(container, currentPlans, available) { rebuildList() }
        }

        buildPlanCards(container, plans, available) { rebuildList() }
    ...
    private fun buildPlanCards(
        ...
        plans: List<UserPreferencesRepository.PlanSave>,
        ...
        for ((i, plan) in plans.withIndex()) {
            ...
            val card = LinearLayout(container.context).apply {
                orientation = LinearLayout.VERTICAL
            ...
            card.addView(TextView(container.context).apply {
                text = plan.name
                textSize = 17f
            ...
            // Action buttons row
            val btnRow = LinearLayout(container.context).apply {
            ...
            listOf("重命名", "读取", "删除").forEach { label ->
                val color = when (label) {
                    "重命名" -> 0xFF0039CB.toInt()
                    "删除" -> 0xFFE3000F.toInt()
                    else -> 0xFF000000.toInt()
                }
                btnRow.addView(android.widget.Button(container.context).apply {
                    text = label
                    textSize = 13f
                    setTextColor(color)
                    ...
                    ).apply { weight = 1f; marginStart = dip(4); marginEnd = dip(4) }
                    setOnClickListener {
                        when (label) {
                            "重命名" -> showRenameDialog(planIndex, plan.name, onRefresh)
                            "读取" -> loadPlan(plan)
                            "删除" -> showDeleteConfirmationDialog(planIndex, plan.name, onRefresh)
                        }
                    }
                })
            }
    ...
    private fun loadPlan(plan: UserPreferencesRepository.PlanSave) {
        ...
        // Go to HomeFragment (not planIndex) so user can choose mode
    ...
    private fun showDeleteConfirmationDialog(index: Int, planName: String, onRefresh: () -> Unit) {
        val nameInput = EditText(this).apply {
            hint = "输入存档名称确认删除"
            setPadding(dip(16), dip(12), dip(16), dip(12))
        }
        ...
        AlertDialog.Builder(this)
            .setTitle("删除存档")
            .setMessage("确定删除「${planName}」吗？此操作不可撤销。\n请输入存档名称确认：")
            .setView(nameInput)
            .setPositiveButton("删除") { _, _ ->
                val input = nameInput.text.toString().trim()
                if (input == planName) {
                    UserPreferencesRepository.deletePlan(index)
                    Toast.makeText(this, "已删除", Toast.LENGTH_SHORT).show()
                    updateContinueButton()
                    onRefresh()
                } else {
                    Toast.makeText(this, "存档名称不匹配", Toast.LENGTH_SHORT).show()
                }
            }
            .setNegativeButton("取消", null)
            .show()
